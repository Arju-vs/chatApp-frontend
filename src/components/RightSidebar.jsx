...
const RightSidebar = () => {
  ...
  const { selectedUser, msges} = useContext(ChatContext)
  const {logout, onlineUsers} = useContext(AuthContext)
  const [msgImages, setMsgImages ] = useState([])

  // get all the images from the msges and set them to state
  useEffect(()=>{
    setMsgImages(
      msges.filter(msg=>msg.image).map(msg=>msg.image)
    )
  },[msges])
    ...
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? "max-md:hidden" : ""} `}>
        <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
          ...
          <h1 className='px-10 text-xl font-medium mx-auto flex items-center'>
            {onlineUsers.includes(selectedUser._id) && <p className='w-2 h-2 mr-1 rounded-full bg-green-500'></p> }
